...
import { publishMeeting, mapsOpenUrl } from '../services/meetingsApi';
...
function resolveVenueCoords(m) {
  let lat = parseCoords(m.venueLat);
  let lng = parseCoords(m.venueLng);
  if (lat != null && lng != null) return { lat, lng };
  const pair = parseLatLngPair(m.googlePlace);
  if (pair) return pair;
...
/**
 * Advanced standalone Meetings app — local device storage only.
 */
const MeetingsApp = () => {
  ...
  const buildMeetingFromForm = (existing) => {
    ...
    if (isInPerson && (venueLat == null || venueLng == null)) {
      setError(
        'In-person meetings need a map pin. Open Place → Map, or switch to Online only.'
      );
      return null;
    }
    const mealMenu = {
    ...
    return {
      ...
      location,
      isInPerson,
      googlePlace: (form.googlePlace || '').trim() || (isInPerson ? location : ''),
  ...
  const onSubmit = async (e) => {
    ...
    for (const inst of instances) {
      let published = !!inst.qrEnabled;
      try {
        await publishMeeting(inst);
        published = true;
        publishedCount += 1;
      } catch {
        published = false;
      }
      savedList.push({ ...inst, qrEnabled: published });
    }
    ...
    if (createSeries) {
      flash(
        `Created ${savedList.length} meetings in the series` +
          (publishedCount
            ? ` · ${publishedCount} ready for check-in QR`
            : ' · enable QR on each card when the API is online') +
          '.'
      );
    } else {
      flash(
        publishedCount
          ? editingId
            ? 'Meeting updated · check-in QR ready.'
            : 'Meeting saved · check-in QR ready.'
          : editingId
            ? 'Meeting updated (open card to enable QR when API is online).'
            : 'Meeting saved (open card to enable QR when API is online).'
      );
    }
    resetForm();
  };
        ...
        <GooglePlacePicker
          ...
          onChange={(place) => {
            ...
            setForm((f) => ({
              ...f,
              googlePlace: label,
              venueLat: Number.isFinite(lat) ? String(lat) : f.venueLat,
              venueLng: Number.isFinite(lng) ? String(lng) : f.venueLng,
              location:
                f.location ||
                (label
            ...
            flash('Venue location saved for check-in verification.');
          }}
        />
      )}
          ...
          <div className="meetings-form-head">
            ...
            <div>
              ...
              <p className="meetings-form-sub">
                Saves for QR check-in when the API is on.
              </p>
            </div>
          </div>
          ...
          <div className="meetings-form-body">
            ...
            {showDetails && (
              <div className="meetings-panel-collapse">
                ...
                <label className="meetings-field">
                  <span>
                    <FaMapMarkerAlt aria-hidden /> Venue
                  </span>
                  <input
                    name="location"
                    className="form-input"
                ...
                <label className="meetings-field">
                  <span>
                    <FaMapMarkerAlt aria-hidden /> Google Maps place
                    {form.isInPerson !== false ? (
                  ...
                  <div className="meetings-place-row">
                    <input
                      ...
                      placeholder={
                        form.isInPerson !== false
                          ? 'Use Map to pin the in-person venue'
                          : 'Optional — pin if useful'
                      }
                      readOnly
                    />
                    <button
                  ...
                  <p className="meetings-field-hint">
                    {form.isInPerson !== false
                      ? `Required for in-person check-in. Guests are checked against this pin (±${form.venueRadiusM || 200} m).`
                      : 'Optional for online meetings. Location verification is off unless you set a pin.'}
                  </p>
                  {form.googlePlace && form.venueLat && form.venueLng && (
                    <p className="meetings-place-preview">
                      <span className="meetings-place-coords">
                        Pinned · {Number(form.venueLat).toFixed(5)},{' '}
                      ...
                      >
                        Open in Google Maps <FaExternalLinkAlt aria-hidden />
                      </a>
                    </p>
                  )}
                  {form.isInPerson !== false &&
                    !(form.venueLat && form.venueLng) && (
                      <p className="meetings-field-hint warn">
                        Map pin required — open Map and confirm the exact venue.
                      </p>
                    )}
                </label>
                {form.isInPerson !== false && (
          ...
          {listView !== 'booking' && (
            ...
            <div className="meetings-empty">
              ...
              <p className="meetings-empty-copy">
                Create a meeting on the left, or switch filters / day on the
                calendar to see more.
              </p>
            </div>
          ) : (
